...
class AIBase(EntityBase):
	...
	###
	def move_to(self, target_obj, behavior=3, offset=0, speed=2):
		...
		# Offset Target
		if offset == 1:
			ry = target_obj.position[0] + self.ryn
			rx = target_obj.position[1] + self.rxn
		else:
			ry = target_obj.position[0]#self.target.position[0] + self.ryn
			rx = target_obj.position[1]#self.target.position[1] + self.rxn
