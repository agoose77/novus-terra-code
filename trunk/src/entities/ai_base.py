...
### 
class AIBase(entities.EntityBase):
	...
	""" 'Misc Functions' """
	def move_to(self, target_pos, behavior=3, offset=0, speed='walk'):
	...
	def attack(self):
		...
		ray.range = 250#self.inventory.current_weapon.range
		hit = ray.hitObject
	...
	def handle_animations(self):
		if self.animations['reload'] == 1:
			...
			self.armature.playAction("pistol_walk", 1, 32, layer=2, priority=2, blendin=5, play_mode=bge.logic.KX_ACTION_MODE_LOOP, speed=1.0)
	...
	def handle_cover(self, FSM):
		...
		in_cover = self.move_to(self.cover.position, offset = 0, speed = 'run')
		self.cover_timer += 1
...
"""
	...
	def __init__(self, packet=None):
		...
		print(packet.game_object)

		### INIT ###
		print("AIBase.__init__()")
		#entities.EntityBase.__init__(self, None)
		#entities.EntityBase._wrap(self, packet.game_object)
	...
	def _wrap(self, object):
		...
		self.target_obj = [child for child in self._data.childrenRecursive if 'target_obj' in child][0]
		self.target_obj.removeParent()

		self.aimer = [child for child in self._data.childrenRecursive if 'aimer' in child][0]
		self.bullet_spread = [child for child in self._data.childrenRecursive if 'spread' in child][0]
		self.weapon_pos = [child for child in self._data.childrenRecursive if 'weapon_pos' in child][0]
