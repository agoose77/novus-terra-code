...
bgui.BGUI_VERTICAL_SCROLLBAR = 1
bgui.BGUI_HORIZONTAL_SCROLLBAR = 2
...
class Scrollbar(bgui.Widget):
	...
	def __init__(self, parent, name, direction=bgui.BGUI_VERTICAL_SCROLLBAR, aspect=None, size=[1,1],\
		...
		if self.theme:
			...
			self._slider.border = float(self.theme.get(self.theme_section, 'SliderBorderSize'))
		else:
			self._slot.colors = [[0.2, 0.2, 0.2, 1.0]] * 4
			self._slider.colors = [[0.4, 0.4, 0.4, 1.0]] * 4
			
			self._slot.border = self._slider.border = 1
